...
function M.play_audio(file_path)
	...
	ui.show_popup("Playing audio...", function()
		M.stop_playback() -- Stop any existing playback
	end)
...
function M.start_recording()
	...
	ui.show_popup("Recording...", function(auto_transcribe)
		M.stop_recording(auto_transcribe)
	end)
	...
	local cmd = {
		...
		"-af", "silencedetect=noise=-30dB:d=2", -- Detect silence of 2s below -30dB
...
---@param auto_transcribe boolean
function M.stop_recording(auto_transcribe)
	...
	vim.defer_fn(function()
		if vim.fn.filereadable(output_file) == 1 then
			...
			-- If auto_transcribe is true, start transcription immediately
			if auto_transcribe then
...
function M.transcribe_audio(file_path, opts)
	...
	ui.show_popup("Transcribing audio...", nil)
